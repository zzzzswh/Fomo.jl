...
在同一套速度模型、观测几何与震源下,Fomo 与 **deepwave**(GPU 有限差分)和
**SPECFEM2D**(谱元法,作精度参考)做了统一的可复现基准:
**[wavebench](https://github.com/zzzzswh/wavebench)**。

- **精度** —— Fomo 对齐谱元参考:声波压力逐道中位相关 ~0.998(L2 ~7%);
  弹性 `vx`/`vz` ~0.95-0.99;走时零延迟。
- **效率** —— 大(带宽受限)网格上,常密度声波用默认变密度 `acoustic2d`
  (一阶速度-应力)比 deepwave 二阶 `scalar` 慢约 2.4-2.6x。这是**数值公式**
  代价(速度-应力每步约 3 倍访存),非实现差距:同公式的 `scalar2d` 与 deepwave
  持平(略快),弹性两码(均为速度-应力)在各网格尺寸下均持平。

完整方法、图与一键复现见 wavebench 仓库。  
